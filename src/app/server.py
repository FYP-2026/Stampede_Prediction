...
DEFAULT_MODEL_PATH = os.path.normpath(os.path.join(
    os.path.dirname(__file__), "..", "model", "heatmap_model",
    "217k_relu", "217k_relu.keras"
))

# ── Density-map loss / metrics (for the 217k_relu heatmap model) ──────────────

def density_loss(lambda_ssim=0.2, lambda_count=0.05):
    """
    Composite Density Map Loss
    Total Loss = MSE + lambda_ssim * SSIM Loss + lambda_count * Count Loss
    """
    def loss(y_true, y_pred):
        ...
        count_loss = tf.reduce_mean(
            tf.abs(pred_count - true_count) / (true_count + 1.0)
        )

        return mse + lambda_ssim * ssim_loss + lambda_count * count_loss
    return loss
...
def count_metric(y_true, y_pred):
    pred_count = tf.reduce_sum(y_pred, axis=[1, 2, 3])
    true_count = tf.reduce_sum(y_true, axis=[1, 2, 3])
    return tf.reduce_mean(tf.abs((pred_count - true_count) / (true_count + 1)))
...
class InferenceWorker:
    ...
    def _run(self):
        self.status = "loading"
        try:
            model_path = self._area.get("model_path", "") or DEFAULT_MODEL_PATH
...
async def _push_density(area_id, count, density, grid_flags):
    ...
    if risk and not area["risk_flagged"] and area["status"] != "closed":
    ...
    elif not risk and area["risk_flagged"] and area["status"] == "closed":
        alert_id = str(uuid.uuid4())
        db.execute(
            "INSERT INTO alerts (id,area_id,area_name,alert_type,density,count) VALUES (?,?,?,?,?,?)",
            (alert_id, area_id, area["name"], "density_safe", density, count)
        )
        db.commit()
        await manager.broadcast_admin({
            "type":       "density_safe",
...
async def my_profile(auth=Depends(verify_token)):
    db = get_db()
    row = db.execute("""
        SELECT v.*, a.name as area_name, a.status as area_status,
               a.density_threshold, a.current_density
        FROM volunteers v LEFT JOIN areas a ON v.area_id = a.id
        WHERE v.id=?
    """, (auth["id"],)).fetchone()
    db.close()
...
@app.post("/volunteers/{volunteer_id}/unassign")
async def unassign_volunteer(volunteer_id: str, auth=Depends(verify_token)):
    if auth["role"] != "admin":
        raise HTTPException(403, "Admin only")
    db = get_db()
    db.execute("UPDATE volunteers SET area_id=NULL WHERE id=?", (volunteer_id,))
    db.commit(); db.close()
    await manager.send_volunteer(volunteer_id, {"type": "area_assigned", "area_id": None})
